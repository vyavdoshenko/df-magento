...
```bash
MAGENTO_URL=http://192.168.0.126 k6 run --duration 1m --vus 50 k6-load-test.js
```
...
```bash
MAGENTO_URL=http://192.168.0.126 k6 run --duration 10m --vus 200 k6-load-test.js
```
...
```bash
MAGENTO_URL=http://192.168.0.126 k6 run --stage 1m:10,30s:200,1m:200,30s:10 k6-load-test.js
```
...
|--------|------------|---------|
| `keyspace_hits` | High (growing) | Cache hits |
| `keyspace_misses` | Low | Cache misses |
| Hit ratio | > 90% | hits / (hits + misses) |
| `used_memory_human` | < 80% of total | Memory usage |
...
2. **Warm up cache:**
   ```bash
   curl http://192.168.0.126/
   curl http://192.168.0.126/women.html
   ```

3. **Check Dragonfly connection:**
   ```bash
   docker exec -it magento_app redis-cli -h dragonfly.service PING
   ```
...
export default function () {
    ...
    const response = http.get('http://192.168.0.126/women/tops-women/jackets-women.html');
...
export function setup() {
    ...
    const loginRes = http.post('http://192.168.0.126/customer/account/loginPost/', {
...
export default function (data) {
    ...
    const response = http.get('http://192.168.0.126/customer/account/', {
...
MAGENTO_URL=http://192.168.0.126 k6 run --out json=results.json k6-load-test.js
...
- [Magento Performance Guide](https://devdocs.magento.com/guides/v2.4/performance-best-practices/)
